...
public class BinaryUploader {
	...
	public static final State save(HttpServletRequest request, Map<String, Object> conf) {
		...
		try {
            ...
            String FASTDFS = "", fastdfsRes = "", fdfs_tracker_prefix = "";
            byte[] imgByte = null;
            try {

				FASTDFS = request.getParameter("WEB_URL_FASTDFS");
				if(!isEmpty(FASTDFS) && FASTDFS.equals("1")){
					fdfs_tracker_prefix = getProp("/fdfs_client.conf");
					fdfs_tracker_prefix ="http://"+fdfs_tracker_prefix.split(":")[0]+":8080";
