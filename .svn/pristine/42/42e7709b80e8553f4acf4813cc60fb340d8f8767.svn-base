...
function addOkListener() {
	dialog.onok = function() {
		...
		//King 20170720
		var urlPrefix = editor.options.WEB_URL,
		...
		for (var i = 0,img; img = images[i++];) {
			...
			for (var j = 0,url; url = imageUrls[j++];) {
				var prefix = url.isFastdfsRes==1?url.tracker_prefix : editor.options.WEB_URL;
                if (src.indexOf(url.original.replace(" ","")) != -1) {
					img.src = prefix + url.url;
					img.setAttribute("_src", prefix + url.url);  //同时修改"_src"属性
